...
public class PokerTableController {
	...
	private static GameRuleDomainModel game_rule = GameRuleBLL.FindDefaultGameRule();
	private static GameRuleCardsDomainModel game_rule_cards = null;

	public static void setGameRuleCards(GameRuleCardsDomainModel game_rule_cards) {
		PokerTableController.game_rule_cards = game_rule_cards;
	}
	public static void setGameRule(GameRuleDomainModel game_rule) {
		PokerTableController.game_rule = game_rule;
	}
	...
	private void handlePlay() {
		...
		DealFaceDownCards(gme.getGameRule().getCOMMUNITYCARDSMAX(), 0);

		// Add a deck to the game
		gme.setGameDeck(new Deck(game_rule.getNUMBEROFJOKERS(), null));
	...
	private void handleDraw() {
		...
		if (act.isbEvaluateHand()) {
			...
			if (iCardDrawnPlayer + iCardDrawnCommon + 2 >= gme.getGameRuleCards().getNBROFCARDS()) {
				for (Player p : mainApp.GetSeatedPlayers()) {
					Hand hPlayer = gme.FindPlayerGame(gme, p).getHand();
					for (int a = hPlayer.getCards().size(); a < gme.getGameRule().getPLAYERNUMBEROFCARDS(); a++) {
					...
					if (hCommon.getCards() == null) {
						for (int a = 0; a < gme.getGameRule().getCOMMUNITYCARDSMAX(); a++) {
							hCommon.AddCardToHand(new Card(eSuit.JOKER, eRank.JOKER, 0));
						}
					} else {

						for (int a = hCommon.getCards().size(); a < gme.getGameRule().getCOMMUNITYCARDSMAX(); a++) {
							hCommon.AddCardToHand(new Card(eSuit.JOKER, eRank.JOKER, 0));
						}
					}
	...
	private void winnersCards(Hand winningHand, Player p) {
		...
		if(gme.getGameRule().getCOMMUNITYCARDSMAX() > 0) {
			for(int i = 0; i < winningHand.getCardsInHand().size(); i++) {
