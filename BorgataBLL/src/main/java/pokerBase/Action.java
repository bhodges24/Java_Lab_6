...
public class Action {
	...
	public Action(GamePlay gme, int iCardDrawnPlayer, int iCardDrawnCommon, int iDrawCount) {
		...
		if (iCardDrawnPlayer < gme.getGameRule().getPLAYERNUMBEROFCARDS()) {
			eDrawAction = eDrawAction.DrawPlayer;
			iCardDrawnPlayer++;
		} else if ((iCardDrawnPlayer >= gme.getGameRule().getPLAYERNUMBEROFCARDS())
				&& (iCardDrawnCommon <= gme.getGameRule().getCOMMUNITYCARDSMAX())) {
			eDrawAction = eDrawAction.DrawCommon;
			iCardDrawnCommon++;
		}
		if (iCardDrawnPlayer + iCardDrawnCommon == gme.getGameRule().getPLAYERNUMBEROFCARDS() + gme.getGameRule().getCOMMUNITYCARDSMAX())
		{
