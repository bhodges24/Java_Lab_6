...
public class Hand extends HandDomainModel {
	...
	public static ArrayList<Hand> ListHands(Hand PlayerHand, Hand CommonHand, GamePlay gme) {
		...
		int iPlayerNumberOfCards = gme.getGameRule().getPLAYERNUMBEROFCARDS();
		int iPlayerCardsMin = gme.getGameRule().getPLAYERCARDSMIN();
		int iPlayerCardsMax = gme.getGameRule().getPLAYERCARDSMAX();
		int iCommonCardsMin = gme.getGameRule().getCOMMUNITYCARDSMIN();
		int iCommonCardsMax = gme.getGameRule().getCOMMUNITYCARDSMAX();
